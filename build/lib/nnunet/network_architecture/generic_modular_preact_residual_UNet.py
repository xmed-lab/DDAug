...
class BasicPreActResidualBlock(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, props, stride=None):
        ...
        self.conv1 = props["conv_op"](
            in_planes, out_planes, kernel_size, padding=[(i - 1) // 2 for i in kernel_size], **kwargs_conv1
        )
    ...
    def __init__(
        ...
        self.convs = nn.Sequential(
            BasicPreActResidualBlock(
                input_channels, output_channels, kernel_size, network_props, first_stride
            ),
            *[
                BasicPreActResidualBlock(output_channels, output_channels, kernel_size, network_props)
...
class PreActResidualUNetDecoder(nn.Module):
    def __init__(
        ...
        self.segmentation_output = nn.Sequential(
            self.segmentation_conv_norm, self.segmentation_conv_nonlin, self.segmentation_output
        )
...
class FabiansPreActUNet(SegmentationNetwork):
    ...
    def __init__(
        ...
        num_features_skips = [
            min(max_features, base_num_features * 2**i) for i in range(expected_num_skips)
        ]
        norm_nonlins = []
...
def find_3d_configuration():
    ...
    conv_op_kernel_sizes = ((3, 3, 3), (3, 3, 3), (3, 3, 3), (3, 3, 3), (3, 3, 3), (3, 3, 3))
    pool_op_kernel_sizes = ((1, 1, 1), (2, 2, 2), (2, 2, 2), (2, 2, 2), (2, 2, 2), (2, 2, 2))
