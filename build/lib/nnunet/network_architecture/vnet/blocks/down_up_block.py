...
class DownBlock(nn.Module):
    ...
    def __init__(
        ...
        self.down_conv = ConvBnAct3d(
            in_channels,
            out_channels,
            kernel_size=down_kernel_size,
            padding=down_padding_size,
            stride=down_stride,
            norm_type=norm_type,
            act_type=act_type,
        )
...
class UpBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.upper == "interpolation":
            self.up_conv = ConvBnAct3d(up_channels, inner_channels, norm_type=norm_type, act_type=act_type)
        elif self.upper == "upsample":
            self.up_conv = nn.Sequential(
                nn.Upsample(scale_factor=2, mode="nearest"),
                ConvBnAct3d(up_channels, inner_channels, norm_type=norm_type, act_type=act_type),
            )
        elif self.upper == "convt":
            self.up_conv = nn.Sequential(
                nn.ConvTranspose3d(up_channels, inner_channels, kernel_size=2, stride=2, bias=False),
                ConvBnAct3d(
                    inner_channels,
                    inner_channels,
                    kernel_size=1,
        ...
        self.trans_conv = ConvBnAct3d(
            inner_channels + ref_channels,
            out_channels,
            kernel_size=1,
            padding=0,
            norm_type=norm_type,
