...
def predict_cases_amos2022(
    ...
    trainer, params = load_model_and_checkpoint_files(
        model, folds, mixed_precision=True, checkpoint_name=checkpoint_name
    )
            ...
            try:
                with torch.no_grad():
                    softmax_resampled = torch.zeros(
                        (softmax.shape[0], *target_shape), dtype=torch.half, device="cuda:0"
                    )
                    ...
                    for c in range(len(softmax)):
                        softmax_resampled[c] = F.interpolate(
                            softmax_gpu[c][None, None], size=target_shape, mode="trilinear"
                        )[0, 0]
                ...
                with torch.no_grad():
                    ...
                    for c in range(len(softmax)):
                        softmax_resampled[c] = F.interpolate(
                            softmax_cpu[c][None, None], size=target_shape, mode="trilinear"
                        )[0, 0]
