...
def export_one_task(
    ...
    copy_pretrained_models_for_task(
        taskname, output_folder, models, nnunet_trainer, nnunet_trainer_cascade, plans_identifier
    )
    copy_ensembles(
        taskname,
        output_folder,
...
def export_pretrained_model(
    ...
    for m in models:
        ...
        assert all([isdir(join(expected_output_folder, i)) for i in expected_folders]), (
            "not all requested folds " "present, " "Task %s model %s" % (task_name, m)
        )

        assert isfile(join(expected_output_folder, "plans.pkl")), "plans.pkl missing, Task %s model %s" % (
            task_name,
            m,
        )
        ...
        if not isfile(join(expected_output_folder, "postprocessing.json")):
            ...
            zipf.write(
                ...
                os.path.relpath(
                    join(expected_output_folder, "postprocessing.json"), network_training_output_dir
                ),
            )
    ...
    for v in valid:
        zipf.write(
            ...
            os.path.relpath(join(ensemble_dir, v, "postprocessing.json"), network_training_output_dir),
        )
    zipf.close()
...
def export_entry_point():
    ...
    parser.add_argument(
        "-tr",
        type=str,
        help="trainer class used for 2d 3d_lowres and 3d_fullres. " "Default: %s" % default_trainer,
        required=False,
    ...
    parser.add_argument(
        "-trc",
        type=str,
        help="trainer class used for 3d_cascade_fullres. " "Default: %s" % default_cascade_trainer,
        required=False,
    ...
    parser.add_argument(
        "-f", nargs="+", help="Folds. Default: 0 1 2 3 4", required=False, default=[0, 1, 2, 3, 4]
    )
