...
def create_custom_splits_for_experiments(task_path):
    data_keys = [
        i[:-4]
        for i in subfiles(
            os.path.join(task_path, "nnUNetData_plans_v2.1_2D_stage0"), join=False, suffix="npz"
        )
    ]
...
if __name__ == "__main__":
    ...
    df_path = (
        "/media/full/tera2/data/challenges/mms/Training-corrected_original/M&Ms Dataset Information.xlsx"
    )
    ...
    split_path_raw_all_ts = os.path.join(
        raw_data_dir, task_name, "splitted_all_timesteps", folder_imagesTr, "split_raw_images"
    )
    split_path_gt_all_ts = os.path.join(
        raw_data_dir, task_name, "splitted_all_timesteps", folder_imagesTr, "split_annotation"
    )
    ...
    select_annotated_frames_mms(
        split_path_raw_all_ts, imagesTr_path, add_zeros=True, is_gt=False, df_path=df_path
    )
    select_annotated_frames_mms(
        split_path_gt_all_ts, labelsTr_path, add_zeros=False, is_gt=True, df_path=df_path
    )
    ...
    json_dict["numTest"] = 0
    json_dict["training"] = [
        {"image": "./imagesTr/%s" % i.split("/")[-1], "label": "./labelsTr/%s" % i.split("/")[-1]}
        for i in labelsTr
    ]
    json_dict["test"] = []
