...
def collect_and_prepare(base_dir, num_processes=12, clean=False):
    ...
    for e in successful:
        if isdir(join(base_dir, "predVal", e)):
            ...
            if len(files) != expected_num_cases:
                print(
                    e,
                    "prediction not done, found %d files, expected %s" % (len(files), expected_num_cases),
                )
                continue
    ...
    output_folder = join(
        base_dir, "predVal_PP", "nnUNetTrainerV2BraTSRegions_DA3_BN__nnUNetPlansv2.1_bs5_15fold"
    )
    maybe_mkdir_p(output_folder)
    threshold = load_pickle(join(out_pp, e, "threshold.pkl"))[2]
    if threshold > 1000:
    ...
    output_folder = join(
        base_dir, "predVal_PP", "nnUNetTrainerV2BraTSRegions_DA4_BN__nnUNetPlansv2.1_bs5_15fold"
    )
    maybe_mkdir_p(output_folder)
    threshold = load_pickle(join(out_pp, e, "threshold.pkl"))[2]
    if threshold > 1000:
...
def score_and_postprocess_model_based_on_rank_then_aggregate():
    ...
    for e in has_val_pred:
        ...
        if len(files) != expected_num_cases_val:
            print(
                e,
                "prediction not done, found %d files, expected %s" % (len(files), expected_num_cases_val),
            )
            continue
