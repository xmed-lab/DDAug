...
def preprocess_single(filename, image_load_path):
    ...
    save_image(
        join(dataset_save_path, image_set, "RibSeg_" + str(id).zfill(4) + "_0000.nii.gz"),
        image,
        spacing=spacing,
        is_seg=False,
    )
...
def load_image(filepath, return_meta=False, is_seg=False):
    ...
    if not return_meta:
        ...
        affine = (
            None  # How do I get the affine transform with SimpleITK? With NiBabel it is just image.affine
        )
