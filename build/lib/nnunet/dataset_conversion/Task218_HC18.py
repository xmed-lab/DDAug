...
if __name__ == "__main__":
    ...
    for filename, pixelsize, _ in csv_content:
        ...
        filename = filename[:-4]
        r.append(
            p.starmap_async(convert_case, ((training_data_folder, filename, pixelsize, imagestr, labelstr),))
        )
    _ = [i.get() for i in r]
    ...
    for filename, pixelsize in csv_content:
        pixelsize = float(pixelsize)
        filename = filename[:-4]
        r.append(p.starmap_async(convert_case, ((test_data_folder, filename, pixelsize, imagests, None),)))
    _ = [i.get() for i in r]
