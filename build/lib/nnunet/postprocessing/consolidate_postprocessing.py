...
def collect_cv_niftis(
    ...
    if not all(exist):
        raise RuntimeError(
            ...
            "The following folds seem to be missing: %s" % [i for j, i in enumerate(folds) if not exist[j]]
        )
...
def consolidate_folds(
    ...
    collect_cv_niftis(output_folder_base, output_folder_raw, validation_folder_name, folds)
    ...
    summary_fold0 = load_json(join(output_folder_base, "fold_0", validation_folder_name, "summary.json"))[
        "results"
    ]["mean"]
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        ...
        type=str,
        required=True,
        help="experiment output folder (fold_0, fold_1, " "etc must be subfolders of the given folder)",
    )
