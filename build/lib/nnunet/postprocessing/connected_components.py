...
def determine_postprocessing(
    ...
    classes = [
        int(i)
        for i in load_json(join(base, raw_subfolder_name, "summary.json"))["results"]["mean"].keys()
        if int(i) != 0
    ]
    ...
    if advanced_postprocessing:
        ...
        for f in fnames:
            ...
            results.append(
                p.starmap_async(load_remove_save, ((predicted_segmentation, output_file, (classes,)),))
            )
    ...
    if any(comp):
        ...
        any_worse = any(
            [
                pp_results["dc_per_class_pp_all"][str(cl)] < pp_results["dc_per_class_raw"][str(cl)]
                for cl in classes
            ]
        )
        if not any_worse:
