...
class nnUNetTrainerV2_5epochs_dummyLoad(nnUNetTrainerV2_5epochs):
    def initialize(self, training=True, force_load_plans=False):
        ...
        self.some_batch = (
            torch.rand((self.batch_size, self.num_input_channels, *self.patch_size)).float().cuda()
        )

        self.some_gt = [
            torch.round(
                torch.rand((self.batch_size, 1, *[int(i * j) for i, j in zip(self.patch_size, k)]))
                * (self.num_classes - 1)
            )
            .float()
            .cuda()
...
class nnUNetTrainerV2_5epochs_dummyLoadCEnoDS(nnUNetTrainerV2_noDeepSupervision):
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        self.some_gt = (
            torch.round(torch.rand((self.batch_size, *self.patch_size)) * (self.num_classes - 1))
            .long()
            .cuda()
        )
