...
class nnUNetTrainerV2_ResencUNet_SimonsInit_DA5(nnUNetTrainerV2_ResencUNet_SimonsInit):
    ...
    def setup_DA_params(self):
        self.deep_supervision_scales = [[1, 1, 1]] + list(
            list(i) for i in 1 / np.cumprod(np.vstack(self.net_num_pool_op_kernel_sizes[1:]), axis=0)
        )[:-1]
        ...
        if self.threeD:
            ...
            self.data_aug_params["rotation_y"] = (-0.0 / 360 * 2.0 * np.pi, 0.0 / 360 * 2.0 * np.pi)
            self.data_aug_params["rotation_z"] = (-0.0 / 360 * 2.0 * np.pi, 0.0 / 360 * 2.0 * np.pi)
        ...
        if self.deep_supervision_scales is not None:
            tr_transforms.append(
                DownsampleSegForDSTransform2(
                    self.deep_supervision_scales, 0, input_key="target", output_key="target"
                )
            )
