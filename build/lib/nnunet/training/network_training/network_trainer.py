...
class NetworkTrainer(object):
    def __init__(self, deterministic=True, fp16=False):
        ...
        self.lr_threshold = (
            1e-6  # the network will not terminate training if the lr is still above this threshold
        )
        ...
        self.save_best_checkpoint = (
            True  # whether or not to save the best checkpoint according to self.best_val_eval_criterion_MA
        )
    ...
    def run_training(self):
        ...
        self.num_batches_per_epoch = BATCHES_PER_EPOCH
        self.max_num_epochs = TOTAL_EPOCHS
        print(f"{self.max_num_epochs=} {self.num_batches_per_epoch=}")
        while self.epoch < self.max_num_epochs:
            ...
            if self.use_progress_bar:
                with trange(self.num_batches_per_epoch, ncols=80) as tbar:
                    for b in tbar:
            ...
            # train loss, val loss, each class dice, class dice average, each tree layer node count, total tree node count
            csv_values = [
                self.all_tr_losses[-1],
                self.all_val_losses[-1],
                *self.avg_foreground_dice,
                sum(self.avg_foreground_dice) / len(self.avg_foreground_dice),
            ]
            csv_str = ",".join(map(lambda each: str(float(each)), csv_values))
            csv_update_str = f"csv update -> epoch: {self.epoch},{csv_str}"

            self.print_to_log_file(csv_update_str)
        ...
        if self.patience is not None:
            ...
            if self.val_eval_criterion_MA > self.best_val_eval_criterion_MA:
                ...
                if self.save_best_checkpoint:
                    ...
                    # if self.epoch > 90:
                    #     self.save_checkpoint(join(self.output_folder, f"z_model_best_E{self.epoch}.model"))
    ...
    def find_lr(self, num_iters=1000, init_value=1e-6, final_value=10.0, beta=0.98):
        ...
        for batch_num in range(1, num_iters + 1):
            ...
            loss = (
                self.run_iteration(self.tr_gen, do_backprop=True, run_online_evaluation=False).data.item() + 1
            )
...
class NetworkTrainer_MCTS(NetworkTrainer):
    ...
    def run_training(self):
        ...
        ############################ MCTS part #############################
        print(f'{"*"*10}using new augment{"*"*10}')
        from nnunet.training.data_augmentation.model import MCTS_Augment

        print("using augment from model")
        print(self.batch_size)

        self.search_augment = MCTS_Augment(
            self.data_aug_params["patch_size_for_spatialtransform"],
            self.data_aug_params,
            self.deep_supervision_scales if self.deep_supervision else None,
            # None
        )
        self.tr_gen = self.search_augment.build_train_gen(self.dl_tr)
        self.tree_folder_dir = self.log_file.replace(".txt", "")
        maybe_mkdir_p(self.tree_folder_dir)
        ############################ MCTS part #############################
        ...
        while self.epoch < self.max_num_epochs:
            ...
            ############################ MCTS part #############################
            tree_update_str, tree_layer_pop_count = self.search_augment.update_epoch(
                1 + self.all_val_losses[-1]
            )
            ############################ MCTS part #############################
            ...
            csv_values = [
                ...
                *self.avg_foreground_dice,
                sum(self.avg_foreground_dice) / len(self.avg_foreground_dice),
                ############################ MCTS part #############################
                *tree_layer_pop_count,
                sum(tree_layer_pop_count),
                ############################ MCTS part #############################
            ]
            csv_str = ",".join(map(lambda each: str(float(each)), csv_values))
            csv_update_str = f"csv update -> epoch: {self.epoch},{csv_str}"
            ...
            ############################ MCTS part #############################
            self.print_to_log_file(tree_update_str)
            self.tr_gen = self.search_augment.build_train_gen(self.dl_tr)
            # self.print_to_log_file(
            #     f"next epoch augment path: [0, 0, 0]\nnew train loader built"
            # )
            
            self.print_to_log_file(
                f"next epoch augment path: {self.search_augment.traverse_path}\nnew train loader built"
            )

            with open(f"{self.tree_folder_dir}/epoch{self.epoch}.json", "w") as fileout:
                json.dump(self.search_augment.to_dict(), fileout, indent=4, ensure_ascii=False)
            with open(f"{self.tree_folder_dir}/epoch{self.epoch}_tree.pkl", "wb") as fileout:
                pickle.dump(self.search_augment, fileout)
            ############################ MCTS part #############################
