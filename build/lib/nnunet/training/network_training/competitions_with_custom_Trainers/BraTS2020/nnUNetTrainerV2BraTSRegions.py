...
from nnunet.training.loss_functions.dice_loss import DC_and_BCE_loss, SoftDiceLoss, get_tp_fp_fn_tn
...
class nnUNetTrainerV2BraTSRegions_Dice(nnUNetTrainerV2BraTSRegions):
    def __init__(
        ...
        self.loss = SoftDiceLoss(
            apply_nonlin=torch.sigmoid, **{"batch_dice": False, "do_bg": True, "smooth": 0}
        )
                ...
                if self.unpack_data:
                    if self.local_rank == 0:
                        ...
                        while not all_present:
                            ...
                            all_present = all(
                                [
                                    isfile(join(self.folder_with_preprocessed_data, i + ".npy"))
                                    for i in case_ids
                                ]
                            )
