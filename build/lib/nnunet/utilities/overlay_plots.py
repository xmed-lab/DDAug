...
def plot_overlay_preprocessed(
    ...
    assert modality_index < (data.shape[0] - 1), "This dataset only supports modality index up to %d" % (
        data.shape[0] - 2
    )
...
def multiprocessing_plot_overlay(
    list_of_image_files, list_of_seg_files, list_of_output_files, overlay_intensity, num_processes=8
):
...
def multiprocessing_plot_overlay_preprocessed(
    list_of_case_files, list_of_output_files, overlay_intensity, num_processes=8, modality_index=0
):
    p = Pool(num_processes)
    r = p.starmap_async(
