...
def ensemble(
    ...
    for f in folds:
        ...
        patient_identifiers1_nii = [
            i[:-7]
            for i in subfiles(validation_folder_net1, False, None, suffix="nii.gz", sort=True)
            if not i.endswith("noPostProcess.nii.gz") and not i.endswith("_postprocessed.nii.gz")
        ]
        patient_identifiers2_nii = [
            i[:-7]
            for i in subfiles(validation_folder_net2, False, None, suffix="nii.gz", sort=True)
            if not i.endswith("noPostProcess.nii.gz") and not i.endswith("_postprocessed.nii.gz")
        ]
