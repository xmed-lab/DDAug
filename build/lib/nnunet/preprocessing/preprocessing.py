...
from nnunet.configuration import RESAMPLING_SEPARATE_Z_ANISO_THRESHOLD, default_num_threads
            ...
            for c in range(data.shape[0]):
                ...
                for slice_id in range(shape[axis]):
                    if axis == 0:
                        ...
                        reshaped_data.append(
                            resize_fn(data[c, :, :, slice_id], new_shape_2d, order, **kwargs).astype(
                                dtype_data
                            )
                        )
                ...
                if shape[axis] != new_shape[axis]:
                    ...
                    if not is_seg or order_z == 0:
                        reshaped_final_data.append(
                            map_coordinates(reshaped_data, coord_map, order=order_z, mode="nearest")[
                                None
                            ].astype(dtype_data)
                        )
                    else:
...
class GenericPreprocessor(object):
    ...
    def _run_internal(
        ...
        min_percent_coverage = (
            0.01  # at least 1% of the class voxels need to be selected, otherwise it may be too sparse
        )
    ...
    def run(
        ...
        all_classes = load_pickle(join(input_folder_with_cropped_npz, "dataset_properties.pkl"))[
            "all_classes"
        ]
