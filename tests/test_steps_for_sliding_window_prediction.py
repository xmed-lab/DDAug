...
class TestSlidingWindow(unittest2.TestCase):
    ...
    def _verify_steps(self, steps, patch_size, image_size, step_size):
        ...
        for dim in range(len(steps)):
            ...
            self.assertTrue(
                all(
                    [steps[dim][i + 1] <= steps[dim][i] + patch_size[dim] for i in range(num_steps[dim] - 1)]
                ),
                "steps are not overlapping or touching. dim: %d, steps:"
                " %s, image_size: %s, patch_size: %s, step_size: "
                "%0.4f" % (dim, str(steps[dim]), str(image_size[dim]), str(patch_size[dim]), step_size),
            )
            ...
            self.assertTrue(
                all(
                    [
                        steps[dim][i] + np.ceil(target_step_sizes_in_voxels[dim]) >= steps[dim][i + 1]
