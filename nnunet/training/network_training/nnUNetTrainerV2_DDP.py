        ...
        for rank in range(world_size):
            ...
            if sample_id_high / self.global_batch_size < (1 - self.oversample_foreground_percent):
                ...
                percent_covered_by_this_rank = (
                    sample_id_high / self.global_batch_size - sample_id_low / self.global_batch_size
                )
                oversample_percent_here = 1 - (
                    ((1 - self.oversample_foreground_percent) - sample_id_low / self.global_batch_size)
                    / percent_covered_by_this_rank
                )
    ...
    def predict_preprocessed_data_return_seg_and_softmax(
        ...
        if do_mirroring:
            assert self.data_aug_params["do_mirror"], (
                "Cannot do mirroring as test time augmentation when training " "was done without mirroring"
            )
