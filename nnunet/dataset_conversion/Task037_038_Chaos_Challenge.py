...
def convert_variant2_predicted_test_to_submission_format(
    ...
    t1_patient_names = [
        i.split("_")[-1][:-7]
        for i in subfiles(folder_with_predictions, prefix="T1", suffix=".nii.gz", join=False)
    ]
    ...
    apply_postprocessing_to_folder(
        final_predictions_folder, postprocessed_folder, for_which_classes, min_valid_obj_size, 8
    )
...
if __name__ == "__main__":
    ...
    json_dict["numTest"] = 0
    json_dict["training"] = [
        {"image": "./imagesTr/%s.nii.gz" % i, "label": "./labelsTr/%s.nii.gz" % i} for i in patient_ids
    ]
    json_dict["test"] = []
