    ...
    for model in models:
        for t in tasks:
            ...
            task_name = subfolders(
                join(network_training_output_dir, model), prefix="Task%03.0d" % t, join=False
            )
            if len(task_name) != 1:
            ...
            for trainer in model_trainers:
                ...
                for v in validation_folders:
                    ...
                    if ok:
                        ...
                        save_json(
                            json_out,
                            join(output_dir, "%s__%s__%s__%s.json" % (task_name, model, trainer, v)),
                        )
                ...
                for v in validation_folders:
                    ...
                    if ok:
                        ...
                        save_json(
                            ...
                            join(
                                output_dir,
                                "%s__%s__%s__%s__%s.json" % (task_name, model, trainer, v, fold_string),
                            ),
                        )
                        foreground_mean2(
                            join(
