...
from nnunet.postprocessing.consolidate_postprocessing import collect_cv_niftis, consolidate_folds
...
def find_task_name(folder, task_id):
    ...
    assert len(candidates) == 1, "more than one candidate for Task id %d found in folder %s" % (
        task_id,
        folder,
    )
    return candidates[0]
            ...
            if disable_postprocessing:
                ...
                if not isfile(join(cv_niftis_folder, "summary.json")):
                    ...
                    summary_fold0 = load_json(
                        join(output_folder, "fold_%d" % folds[0], validation_folder, "summary.json")
                    )["results"]["mean"]
            ...
            if len(valid_models) > 1:
                for m1, m2 in combinations(valid_models, 2):
                    ...
                    output_folder_base = join(
                        network_training_output_dir, "ensembles", id_task_mapping[t], ensemble_name
                    )
            ...
            if v == best:
                ...
                if k.startswith("ensemble"):
                    ...
                    if not disable_postprocessing:
                        ...
                        predict_str += (
                            "nnUNet_ensemble -f OUTPUT_FOLDER_MODEL1 OUTPUT_FOLDER_MODEL2 -o OUTPUT_FOLDER\n"
                        )
                else:
                    predict_str += (
                        "nnUNet_predict -i FOLDER_WITH_TEST_CASES -o OUTPUT_FOLDER_MODEL1 -tr "
