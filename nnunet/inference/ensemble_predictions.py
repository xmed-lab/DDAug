...
def merge_files(files, properties_files, out_file, override, store_npz):
    if override or not isfile(out_file):
        ...
        reg_class_orders = [
            p["regions_class_order"] if "regions_class_order" in p.keys() else None for p in props
        ]
        ...
        save_segmentation_nifti_from_softmax(
            softmax, out_file, props[0], 3, regions_class_order, None, None, force_separate_z=None
        )
        if store_npz:
...
def merge(folders, output_folder, threads, override=True, postprocessing_file=None, store_npz=False):
    ...
    if postprocessing_file is not None:
        ...
        apply_postprocessing_to_folder(
            output_folder, output_folder_orig, for_which_classes, min_valid_obj_size, threads
        )
