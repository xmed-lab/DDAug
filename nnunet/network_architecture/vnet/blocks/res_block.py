...
class BottleNeck(BasicResBlock):
    ...
    def __init__(
        ...
        for i in range(num_layer):
            if i == num_layer - 1:
                conv = BottConvBnAct3d(
                    channels, ratio=ratio, norm_type=norm_type, act_type=act_type, final_act=False
                )
            else:
                conv = BottConvBnAct3d(
                    channels, ratio=ratio, norm_type=norm_type, act_type=act_type, final_act=True
                )
            layers.append(conv)
        if se_type == "cse":
...
class ResBlock(BasicResBlock):
    ...
    def __init__(self, channels, norm_type=nn.BatchNorm3d, act_type=nn.ReLU, se_type=None, num_layer=1):
        ...
        assert se_type in [
            ...
            "scse",
            None,
        ], "se_type should be in ['cse', 'sse', 'scse', None]"
        layers = []
