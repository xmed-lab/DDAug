...
class SegmentationNetwork(NeuralNetwork):
    ...
    def _internal_predict_2D_2Dconv(
        ...
        assert self.input_shape_must_be_divisible_by is not None, (
            "input_shape_must_be_divisible_by must be set to " "run _internal_predict_2D_2Dconv"
        )
        if verbose:
            print("do mirror:", do_mirroring)

        data, slicer = pad_nd_image(
            x, min_size, pad_border_mode, pad_kwargs, True, self.input_shape_must_be_divisible_by
        )
    ...
    def _internal_predict_3D_3Dconv(
        ...
        assert self.input_shape_must_be_divisible_by is not None, (
            "input_shape_must_be_divisible_by must be set to " "run _internal_predict_3D_3Dconv"
        )
        if verbose:
            print("do mirror:", do_mirroring)
