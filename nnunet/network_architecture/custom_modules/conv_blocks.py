    ...
    def __init__(self, in_planes, out_planes, kernel_size, props, stride=None, use_avgpool_in_skip=False):
        ...
        if (stride is not None and any([i != 1 for i in stride])) or (in_planes != out_planes):
            if use_avgpool_in_skip:
                ...
                ops.append(
                    props["conv_op"](in_planes, out_planes, kernel_size=1, padding=0, stride=1, bias=False)
                )
...
class ResidualLayer(nn.Module):
    def __init__(
        ...
        if block == ResidualBottleneckBlock:
            ...
            self.convs = nn.Sequential(
                block(
                    input_channels, output_channels, kernel_size, network_props, first_stride, **block_kwargs
                ),
                *[
                    block(output_channels, output_channels, kernel_size, network_props, **block_kwargs)
                    for _ in range(num_blocks - 1)
                ]
            )
